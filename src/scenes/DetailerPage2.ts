...
export default class DetailerPage2 extends Phaser.Scene {
	...
	editorCreate(): void {
		...
		// detailer_value
		const detailer_value = this.add.image(511, 382, "detailer_page_2");

		// detailerHeader
		const detailerHeader = new DetailerHeader(this, 409, 45);
		this.add.existing(detailerHeader);

		// homeButton
		const homeButton = new HomeButton(this, 69, 698);
	...
	create() {
		...
		this.nextBtn.on("pointerdown", () => this.scene.start("DetailerPage3"));
		this.prevBtn.on("pointerdown", () => this.scene.start("DetailerPage1"));
	}
